...
void list_push( struct List* list, int val, struct List* g_nodeFreeList)
{
  struct Node* newNode;
...
int list_pop( struct List* list, struct List* g_nodeFreeList)
{
...
void list_clear( struct List* list, struct List* g_nodeFreeList)
{
  while ( list_getSize(list) > 0 )
    list_pop(list, g_nodeFreeList);
}
...
void towers_init( struct Towers* this, int n, struct List* g_nodeFreeList)
{
  int i;
  ...
  for ( i = 0; i < n; i++ )
    list_push( &(this->pegA), n-i, g_nodeFreeList);

}

void towers_clear( struct Towers* this, struct List* g_nodeFreeList)
{

  list_clear( &(this->pegA), g_nodeFreeList);
  list_clear( &(this->pegB), g_nodeFreeList);
  list_clear( &(this->pegC), g_nodeFreeList);

  towers_init( this, this->numDiscs, g_nodeFreeList);
                     ...
                     struct List* g_nodeFreeList)
{
  ...
  if ( n == 1 ) {
    val = list_pop(startPeg, g_nodeFreeList);
    list_push(destPeg,val, g_nodeFreeList);
    this->numMoves++;
  }
  else {
    towers_solve_h( this, n-1, startPeg, destPeg,  tempPeg, g_nodeFreeList);
    towers_solve_h( this, 1,   startPeg, tempPeg,  destPeg, g_nodeFreeList);
    towers_solve_h( this, n-1, tempPeg,  startPeg, destPeg, g_nodeFreeList);
  }

}

void towers_solve( struct Towers* this, struct List* g_nodeFreeList)
{
  towers_solve_h( this, this->numDiscs, &(this->pegA), &(this->pegB), &(this->pegC), g_nodeFreeList);
}
...
int main( int argc, char* argv[] )
{
  ...
  towers_init( &towers, NUM_DISCS, &g_nodeFreeList);

  // If needed we preallocate everything in the caches

#if PREALLOCATE
  towers_solve( &towers, &g_nodeFreeList);
#endif

  // Solve it

  towers_clear( &towers, &g_nodeFreeList);
  setStats(1);
  towers_solve( &towers, &g_nodeFreeList);
  setStats(0);
