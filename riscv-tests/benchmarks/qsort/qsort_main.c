...
int main( int argc, char* argv[] )
{
  ...
  int i;
  type input_dataX[DATA_SIZE];
  for(i = 0; i < DATA_SIZE; i++) {
    input_dataX[i] = input_data[i];
  }

#if PREALLOCATE
  // If needed we preallocate everything in the caches
  sort(DATA_SIZE, input_dataX);
  if (verify(DATA_SIZE, input_dataX, verify_data))
    return 1;
#endif
  ...
  // Check the results
  return verify( DATA_SIZE, input_dataX, verify_data);
}
