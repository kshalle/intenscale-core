    ...
    extends BaseTile(params, x, HartsWontDeduplicate(params), q)
{
  ...
  val ceaseNode: Seq[IntOutwardNode] = 0 to NUM_PUS*NUM_CTXT - 1 map { x => IntIdentityNode()}
  val haltNode: Seq[IntOutwardNode] = 0 to NUM_PUS*NUM_CTXT - 1 map { x => IntIdentityNode()}
  val wfiNode: Seq[IntOutwardNode] = 0 to NUM_PUS*NUM_CTXT - 1 map { x => IntIdentityNode()}
