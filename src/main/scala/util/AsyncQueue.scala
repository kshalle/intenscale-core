...
object AsyncQueueParams {
  ...
  def singleton(sync: Int = 3, safe: Boolean = true, depth: Int = 1) = AsyncQueueParams(depth, sync, safe, false)
}
...
class SyncValidSync(sync: Int, desc: String) extends RawModule {
  val io = IO(new Bundle {
    val in = Input(Bool())
    val out = Output(Bool())
  })
...
class AsyncQueueSource[T <: Data](gen: T, params: AsyncQueueParams = AsyncQueueParams()) extends Module {
  ...
  io.async.safe.foreach { sio =>
    ...
    val source_valid_1 = Module(new SyncValidSync(params.sync, "source_valid_1"))
    ...
    source_valid_1.reset := !source_valid_0.io.out
...
class AsyncQueueSink[T <: Data](gen: T, params: AsyncQueueParams = AsyncQueueParams()) extends Module {
  ...
  io.async.safe.foreach { sio =>
    ...
    val sink_valid_1    = Module(new SyncValidSync(params.sync, "sink_valid_1"))
    ...
    sink_valid_1 .reset := !sink_valid_0.io.out
...
class AsyncQueueRegnode[T <: Data](gen: T, params: AsyncQueueParams = AsyncQueueParams()) extends Crossing[T]{
  val io = IO(new CrossingIO(gen))
  val source = Module(new AsyncQueueSourceRegnode(gen, params))
  val sink   = Module(new AsyncQueueSinkRegnode  (gen, params))
...
class AsyncQueueSourceRegnode[T <: Data](gen: T, params: AsyncQueueParams = AsyncQueueParams()) extends Module{
  val io = IO(new Bundle {
    // These come from the source domain
    val enq = Flipped(Decoupled(gen))
    // These cross to the sink clock domain
...
class AsyncQueueSinkRegnode[T <: Data](gen: T, params: AsyncQueueParams = AsyncQueueParams()) extends Module{
  val io = IO(new Bundle {
    // These come from the sink domain
    val deq = Decoupled(gen)
    // These cross to the source clock domain
