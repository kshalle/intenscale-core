...
class BaseSubsystemConfig extends Config ((site, here, up) => {
  ...
  case MaxHartIdBits => val params = site(IntenCoreKey) 
                        log2Up(site(RocketTilesKey).size * params.NUM_L2MACRO)
  ...
  case MemoryBusKey => val params = site(IntenCoreKey)
    MemoryBusParams(
    doexist = !params.BACKEND_ENA,
    beatBytes = site(XLen)/8,
    blockBytes = site(CacheBlockBytes))
...
class WithNBigCores extends Config((site, here, up) => {
  case RocketTilesKey => {
    val big = RocketTileParams(
      ...
      icache = Some(ICacheParams(
        rowBits = site(SystemBusKey).beatBits,
        blockBytes = site(CacheBlockBytes))))
    val params = site(IntenCoreKey)
    List.tabulate(params.NUM_CTXT * params.NUM_PUS * params.NUM_PHY_CORES)(i => big.copy(hartId = i))
  }
})
...
class WithDefaultMemPort extends Config((site, here, up) => {
  case ExtMem => Some(MemoryPortParams(MasterPortParams(
                      ...
                      idBits = 4), site(IntenCoreKey).NUM_MEMORYCHANNELS))
})

class WithPhysAddrBits extends Config((site, here, up) => {
  case ExtMem => Some(MemoryPortParams(MasterPortParams(
                      base = x"2000_0000",
                      size = BigDecimal(math.pow(2,site(IntenCoreKey).NUM_PHYS_ADDR_BITS)).toBigInt - x"2000_0000",
                      beatBytes = site(MemoryBusKey).beatBytes,
                      idBits = 4), site(IntenCoreKey).NUM_MEMORYCHANNELS))
})
