...
case class CoherentBusTopologyParams(
  ...
  instantiations = (if (mbus.doexist == false) List(
    (L2, CoherenceManagerWrapperParams(mbus.blockBytes, mbus.beatBytes, l2.nBanks, L2.name)(l2.coherenceManager))) else List(
    (MBUS, mbus),
  ...
  connections = if (l2.nBanks == 0) Nil else List(
    //(SBUS, L2,   TLBusWrapperConnection(driveClockFromMaster = Some(true), nodeBinding = BIND_STAR)()), // Diconnect L2 from SBUS.
