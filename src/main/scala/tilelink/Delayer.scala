...
class FixTLDelayer(q: Int)(implicit p: Parameters) extends LazyModule
{
  val node = TLAdapterNode()

  lazy val module = new LazyModuleImp(this) {
    def feed[T <: Data](sink: DecoupledIO[T], source: DecoupledIO[T], noise: T) {
      val allow = UInt(q) <= LFSRNoiseMaker(3, source.valid)
      val cycle_count = RegInit(0.U(2.W))
      cycle_count := cycle_count + 1.U;
      sink.valid := source.valid && (cycle_count === 0.U)
      source.ready := sink.ready && (cycle_count === 0.U)
      sink.bits := source.bits
      when (!sink.valid) { sink.bits := noise }
    }
...
object FixTLDelayer
{
  def apply(q: Int)(implicit p: Parameters): TLNode =
  {
    val delayer = LazyModule(new FixTLDelayer(q))
    delayer.node
  }
}