...
object TLArbiter
{
  ...
  val roundRobinRev: Policy = (width, valids, select) => if (width == 1) UInt(1, width=1) else {
    val valid = Reverse(valids(width-1, 0))
    assert (Reverse(valid) === valids)
    ...
    //  mask := leftOR(readys & valid, width)
    //}
    //Reverse(readys(width-1, 0))

    val mask = RegInit(UInt(1, width=width))
    val defRet  = ~(leftOR(valids) << 1)(width-1, 0)
    when(select) {
      mask := Cat(mask(width-2, 0), mask(width-1))
    }
    Mux((mask & valids) =/= UInt(0), mask, defRet)
  }
  ...
  def robinX[T <: TLChannel](edge: TLEdge, sink: DecoupledIO[T], sources: Seq[DecoupledIO[T]]) {
    apply(roundRobinRev)(sink, sources.map(s => (edge.numBeats1(s.bits), s)):_*)
  }
