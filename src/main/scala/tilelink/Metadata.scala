...
class ClientMetadata extends Bundle {
  ...
  def onAccessFast(c: UInt): Bool = {
    // Note: the output is not correct in the case state==Nothing.
    import MemoryOpCategories._
    import ClientStates._

    // A flag to indicates we do not need to change the state according to the rules in growStarter nor ask for permission.
    // c(0) is set for wi and wr. c(1) is set for wi only.
    !((state===Branch) && c(0)) && !((state===Trunk) && c(1))
  }
