...
class TLFilter(
  //mfilter is manager filter -- manager is slave
  mfilter: TLFilter.ManagerFilter = TLFilter.mIdentity,
  //cfilter is client -- client is master (client / manager is old TL terminology)
  cfilter: TLFilter.ClientFilter  = TLFilter.cIdentity
  )(implicit p: Parameters) extends LazyModule
{
  //AdapterNode is used for caches -- contains Diplomacy negotiation code -- no hardware here, it's outer twin -- gates are created in inner twin (below)
  val node = TLAdapterNode(
    ...
    //This defines the function that manages the Diplomacy
    managerFn = { mp =>
  ...
  //The Inner Twin is where hardware is created -- there are only wires here, so filter has no gates inside
