...
import freechips.rocketchip.util.{AsyncQueue,AsyncQueueRegnode,AsyncQueueParams,AsyncResetRegVec}
...
class pcieWriteCrossingIO[T <: Data](gen: T) extends Bundle {
  // Master clock domain
  val master_clock   = Clock(INPUT)
  val master_reset   = Input(AsyncReset())
  val master_port    = gen.asInput
  // Slave clock domain
  val slave_clock    = Clock(INPUT)
  val slave_reset    = Input(AsyncReset())
  val slave_port     = gen.asOutput
}

class pcieWriteCrossing[T <: Data](gen: T, sync: Int = 3) extends Module{
  val io = new pcieWriteCrossingIO(gen)
  // The crossing must only allow one item inflight at a time

  val crossing = Module(new AsyncQueueRegnode(gen, AsyncQueueParams.singleton(sync, depth=8)))

  crossing.io.enq_clock := io.master_clock
  crossing.io.enq_reset := io.master_reset.asAsyncReset
  crossing.io.deq_clock := io.slave_clock
  crossing.io.deq_reset := io.slave_reset.asAsyncReset

  crossing.io.enq.bits := io.master_port
  crossing.io.enq.valid := Bool(true)

  crossing.io.deq.ready := Bool(true)
  io.slave_port := crossing.io.deq.bits
