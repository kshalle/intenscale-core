...
object RegMapper
{
  ...
  def apply(bytes: Int, concurrency: Int, undefZero: Boolean, in: DecoupledIO[RegMapperInput], mapping: RegField.Map*)(implicit sourceInfo: SourceInfo) = {
    ...
    // Get sequence of all registers.
    val allRegs = wordmap.toList.map { case (word, fields) => regIndexI(word)}.distinct.sorted
    val numRegs = allRegs.size

    // Create an address map.
    var addrMap  = Array(BitPat(allRegs(0)) -> List(BitPat(0), Y))
    for (i <- 1 until numRegs) {
      addrMap  = addrMap  :+ (BitPat(allRegs(i)) -> List(BitPat(i), Y))
    }

    // Protection flag for undefined registers
    val iRightReg = Array.fill(numRegs) { Bool(true) }
    val oRightReg = Array.fill(numRegs) { Bool(true) }
    ...
    val flat = wordmap.toList.map { case (word, fields) =>
      ...
      if (undefZero) {
        ...
        val regId = allRegs.indexOf(index)
        iRightReg(regId) = findex === uint
        oRightReg(regId) = bindex === uint
      }
    ...
    val rifire = Array.fill(numRegs) { Nil:List[(Bool, Bool)] }
    val wifire = Array.fill(numRegs) { Nil:List[(Bool, Bool)] }
    val rofire = Array.fill(numRegs) { Nil:List[(Bool, Bool)] }
    val wofire = Array.fill(numRegs) { Nil:List[(Bool, Bool)] }

    // The output values for each register
    val dataOut = Array.fill(numRegs) { UInt(0) }
    ...
    for (i <- 0 until flat.size) {
      ...
      rifire(regId) = (rivalid(i), litOR(f_riready, !rimask)) +: rifire(regId)
      wifire(regId) = (wivalid(i), litOR(f_wiready, !wimask)) +: wifire(regId)
      rofire(regId) = (roready(i), litOR(f_rovalid, !romask)) +: rofire(regId)
      wofire(regId) = (woready(i), litOR(f_wovalid, !womask)) +: wofire(regId)

      // ... this loop iterates from smallest to largest bit offset
      val prepend = if (low == 0) { f_data } else { Cat(f_data, dataOut(regId) | UInt(0, width=low)) }
      dataOut(regId) = (prepend | UInt(0, width=high+1))(high, 0)
    }
    ...
    val iindexX  = DecodeLogic(iindex, List(BitPat(0), N), addrMap)
    val iindexM  = iindexX(0)
    val iindexV  = Wire(Bool())
    iindexV     := iindexX(1)
    val frontSel = UIntToOH(iindexM).asBools

    val oindexX  = DecodeLogic(oindex, List(BitPat(0), N), addrMap)
    val oindexM  = oindexX(0)
    val oindexV  = Wire(Bool())
    oindexV     := oindexX(1)
    val backSel  = UIntToOH(oindexM).asBools
    ...
    val rifireMux = mux(iindexM, in.valid && front.ready &&  front.bits.read, frontSel, iRightReg, rifire)
    val wifireMux = mux(iindexM, in.valid && front.ready && !front.bits.read, frontSel, iRightReg, wifire)
    val rofireMux = mux(oindexM, back.valid && out.ready &&  back .bits.read, backSel,  oRightReg, rofire)
    val wofireMux = mux(oindexM, back.valid && out.ready && !back .bits.read, backSel,  oRightReg, wofire)

    val iready = Mux(front.bits.read, rifireMux, wifireMux) || !iindexV
    val oready = Mux(back .bits.read, rofireMux, wofireMux) || !oindexV
    ...
    out.bits.data  := Mux(MuxTree(oindexM, oRightReg) && oindexV, MuxTree(oindexM, dataOut), UInt(0))
