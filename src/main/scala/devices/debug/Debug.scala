...
class TLDebugModuleInner(device: Device, getNComponents: () => Int, beatBytes: Int)(implicit p: Parameters) extends LazyModule
{
  ...
  lazy val module = new LazyModuleImp(this){
    ...
    val haltedBitRegs    = RegInit(VecInit(Seq.fill(nComponents)(false.B)))
    val resumeReqRegs    = RegInit(VecInit(Seq.fill(nComponents)(false.B)))
    val haveResetBitRegs = RegInit(VecInit(Seq.fill(nComponents)(false.B)))
