...
case class AddressSet(base: BigInt, mask: BigInt) extends Ordered[AddressSet]
{
  ...
  // THIS IS A HACK. We combine the small fragments into a big one.
  // The arbitary chosen 0xfff shall equal or bigger than 2 ** (blockOffBits + log2(NUM_L2BANKS)) - 1.
  def maskHack      = if(mask > 0xb00) (mask | 0xfff) else mask
  def alignmentHack = ((maskHack + 1) & ~maskHack)

  def contains(x: BigInt) = ((x ^ base) & ~mask) == 0                               // Used for elaboration.
  def contains(x: UInt)   = ((x ^ UInt(base)).zext() & SInt(~maskHack)) === SInt(0) // Used for the HW.

  // turn x into an address contained in this set
  def legalize(x: UInt): UInt = base.U | (maskHack.U & x)
  ...
  def toRanges = {
    ...
    val size = alignmentHack // This is uded to build the device tree, and we need to avoid list the fragments.
    val fragments = maskHack & ~(size-1)
