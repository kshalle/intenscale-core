    ...
    dtsCompat: Option[Seq[String]] = None)(implicit p: Parameters) extends LazyModule
{
  ...
  def makeSinglePortedByteWriteSeqMem(size: BigInt, lanes: Int = beatBytes, bits: Int = 8, suffix: String = "") = {
    // We require the address range to include an entire beat (for the write mask)

    val (mem, omSRAM) =  DescribedSRAM(
      name = devName.getOrElse("mem") + suffix,
      desc = devName.getOrElse("mem") + suffix,
      size = size,
