...
trait HasLazyRoCC extends CanHavePTW { this: BaseTile =>
  ...
  // TODO: Connect the ROCC to the correct mini-cache.
  roccs.map(_.atlNode).foreach { atl => tlMasterXbar(0).node :=* atl }
  roccs.map(_.tlNode).foreach { tl => tlOtherMastersNode(0)  :=* tl }
