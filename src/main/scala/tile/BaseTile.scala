...
/** These parameters values are not computed based on diplomacy negotiation
  ...
  */
trait HasNonDiplomaticTileParameters extends HasIntenParameters {
...
/** These parameters values are computed based on diplomacy negotiations
  ...
  */
trait HasTileParameters extends HasNonDiplomaticTileParameters with HasIntenParameters {
    ...
    with HasNonDiplomaticTileParameters
{
  ...
  def intInwardNode: ListBuffer[IntInwardNode]    // Interrupts to the core from external devices
  def intOutwardNode: ListBuffer[IntOutwardNode]  // Interrupts from tile-internal devices (e.g. BEU)
  ...
  protected val tlOtherMastersNode  = 0 to NUM_L2BANKS - 1 map { x => TLIdentityNode()}
  protected val tlMasterXbar        = 0 to NUM_L2BANKS - 1 map { x => LazyModule(new TLXbar)}
  protected val icacheNode          = 0 to NUM_PUS - 1 map { x => TLIdentityNode()}
  protected val dcacheNode          = 0 to NUM_L2BANKS - 1 map { x => TLIdentityNode()}
  protected val cacheXbar           = 0 to NUM_L2BANKS - 1 map { x => LazyModule(new TLXbar)}
  protected val masterNodecachable  = 0 to NUM_PUS - 1 map { x => TLIdentityNode()}
  // Per-L2-bank async crossing for dcache master traffic, bound to masterNode
  // in RocketTile.scala. Giving each bank its own independent crossing avoids
  // funneling all banks through one shared merge/fan-out adapter, which would
  // bottleneck under load.
  val masterNodeRatCross = 0 to NUM_L2BANKS - 1 map { x => LazyModule(new TLAsyncCrossing()(p)) }
  protected val tlMasterXbarcacheable = LazyModule(new TLXbar)
  ...
  def crossMasterCachablePort(ips: Int, crossing1: RocketCrossingParams): TLOutwardNode = {
    val tlMasterXing = this.crossOut(crossing match {
      case RationalCrossing(_) => this { makeMasterBoundaryBuffers } := masterNodecachable(ips)
      case _ => masterNodecachable(ips)
    })
    tlMasterXing(crossing1.crossingType)
  }
  ...
  def crossIntIn(ctxtId: Int): IntInwardNode = crossIntIn(intInwardNode(ctxtId))
  def crossIntOut(ctxtId: Int): IntOutwardNode = crossIntOut(intOutwardNode(ctxtId))

  // TLAsyncCrossing, not TLRationalCrossing: the tile (clk_3_5GHz) and L2Macro
  // (clk_1_75GHZ) sides are a real 2:1 crossing, not a fixed-ratio one.
  val mmioSlaveNode = LazyModule(new TLAsyncCrossing()(p))
  val PrefetcherNode = LazyModule(new TLAsyncCrossing()(p))
...
abstract class BaseTileModuleImp[+L <: BaseTile](val outer: L) extends LazyModuleImp(outer) with HasTileParameters {
  ...
  val traceOut = IO((new TracedInstruction).asOutput)
  // val traceIn  = IO((UInt()).asInput)
}
