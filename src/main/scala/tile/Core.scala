...
trait HasCoreParameters extends HasTileParameters {
  ...
  // Get the nWays used for the ICache.
  val cacheParamsI = tileParams.icache.get
  def nWaysIC      = cacheParamsI.nWays

  // The ID of the bit used to divide the ICache into two halves in the case of NUM_PUS == 2.
  val IC_HLF_BIT     = CACHE_LINE_LOG2 + log2Up(nWaysIC) - 1
}
