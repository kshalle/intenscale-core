...
import chisel3.experimental.{ExtModule, HasExtModuleInline}
...
private class MemRWHelper extends MemHelper {
...
abstract class DifftestMem(size: BigInt, lanes: Int, bits: Int) extends Module {
  ...
  val io = IO(new Bundle {
    ...
    val write = new Bundle {
      val valid = Input(Bool())
      val index = Input(UInt(64.W))
      val data = Input(Vec(n_helper, UInt(64.W)))
      val mask = Input(Vec(n_helper, UInt(64.W)))
    }
  })
  val read = io.read
  val write = io.write
