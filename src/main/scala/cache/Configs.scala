...
class WithInclusiveCache(
  ...
  case BankedL2Key => up(BankedL2Key, site).copy(coherenceManager = { context =>
    ...
    // CoherenceManagerInstantiationFn here returns Option[IntOutwardNode], dropping the
    // two TLOutwardNode entries from the upstream (TLOutwardNode, TLOutwardNode,
    // Option[IntOutwardNode]) tuple type, to match this call site's expected signature.
    None: Option[IntOutwardNode]
  })
})
