...
class BreakpointUnit(n: Int)(implicit val p: Parameters) extends Module with HasCoreParameters {
  ...
  (io.bpwatch zip io.bp).foldLeft((true.B, true.B, true.B)) { case ((ri, wi, xi), (bpw, bp)) =>
    ...
    val r = en && bp.control.r && bp.addressMatch(io.ea) && io.ea_valid
    val w = en && bp.control.w && bp.addressMatch(io.ea) && io.ea_valid
    val x = en && bp.control.x && bp.addressMatch(io.pc) && io.pc_valid
