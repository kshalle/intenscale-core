...
class DCacheModule(outer: DCache) extends HellaCacheModule(outer) {
  ...
  val clock_en_reg = Reg(init=Bool(false))
  ...
  val tl_out_a = Wire(tl_out(0).a)
  tl_out(0).a <> outer.bufferUncachedRequests
  ...
  val (tl_out_c, release_queue_empty) =
    if (cacheParams.acquireBeforeRelease) {
      val q = Module(new Queue(tl_out(0).c.bits.cloneType, cacheDataBeats, flow = true))
      tl_out(0).c <> q.io.deq
  ...
  val s1_probe = Reg(next=tl_out(0).b.fire(), init=Bool(false))
  val probe_bits = RegEnable(tl_out(0).b.bits, tl_out(0).b.fire()) // TODO has data now :(
  ...
  val s1_flush_valid = Reg(init=Bool(false))
  ...
  when (s1_valid && s1_readwrite && tlb.io.respD1.miss) { s1_nack := true }
  ...
  val s1_paddr = tlb.io.respD1.paddr
  ...
  val s1_all_data_ways = Vec(data.io.resp :+ dummyEncodeData(tl_out(0).d.bits.data))
  ...
  val s2_data = {
    ...
    if (cacheParams.pipelineWayMux) {
      ...
      RegEnable(Mux1H(s1_data_way, s1_all_data_ways), en || tl_out(0).d.fire())
    }
  }
  ...
  // grant
  val (d_first, d_last, d_done, d_address_inc) = edge.addr_inc(tl_out(0).d)
  val (d_opc, grantIsUncached, grantIsUncachedData) = {
    ...
    val whole_opc = tl_out(0).d.bits.opcode
    if (usingDataScratchpad) {
      assert(!tl_out(0).d.valid || whole_opc.isOneOf(uncachedGrantOpcodes))
  ...
  tl_out(0).d.ready := Mux(grantIsCached, (!d_first || tl_out(0).e.ready) && canAcceptCachedGrant, true.B)
  when (tl_out(0).d.fire()) {
    when (grantIsCached) {
      ...
      val d_sel = UIntToOH(tl_out(0).d.bits.source, maxUncachedInFlight+mmioOffset) >> mmioOffset
  ...
  tl_out(0).e.valid := tl_out(0).d.valid && d_first && grantIsCached && canAcceptCachedGrant
  tl_out(0).e.bits := edge.GrantAck(tl_out(0).d.bits)
  assert(tl_out(0).e.fire() === (tl_out(0).d.fire() && d_first && grantIsCached))
  ...
  dataArb.io.in(1).valid := tl_out(0).d.valid && grantIsRefill && canAcceptCachedGrant
  when (grantIsRefill && !dataArb.io.in(1).ready) {
    tl_out(0).e.valid := false
    tl_out(0).d.ready := false
  }
  if (!usingDataScratchpad) {
    ...
    dataArb.io.in(1).bits.wdata := tl_out(0).d.bits.data
    dataArb.io.in(1).bits.poison := tl_out(0).d.bits.corrupt
  ...
  metaArb.io.in(3).valid := grantIsCached && d_done && !tl_out(0).d.bits.denied
  ...
  metaArb.io.in(3).bits.data := tECC.encode(L1Metadata(s2_req.addr >> tagLSB, s2_hit_state.onGrant(s2_req.cmd, tl_out(0).d.bits.param)).asUInt)

  // don't accept uncached grants if there's a structural hazard on s2_data...
  val blockUncachedGrant = Reg(init=Bool(false))
  ...
  ccover(tl_out(0).d.valid && !tl_out(0).d.ready, "BLOCK_D", "D$ D-channel blocked")
  ...
  metaArb.io.in(6).valid := tl_out(0).b.valid && (!block_probe || lrscBackingOff)
  tl_out(0).b.ready := metaArb.io.in(6).ready && !block_probe && !s1_valid && !s2_valid
  metaArb.io.in(6).bits.write := false
  metaArb.io.in(6).bits.idx := probeIdx(tl_out(0).b.bits)
  metaArb.io.in(6).bits.addr := Cat(io.cpu.req.bits.addr >> paddrBits, tl_out(0).b.bits.address)
  ...
  if (usingDataScratchpad) {
    ...
    ccover(tl_out(0).b.valid && !tl_out(0).b.ready, "BLOCK_B", "D$ B-channel blocked")
  }

  // uncached response
  val replay_next = tl_out(0).d.fire() && grantIsUncachedData
  val doUncachedResp = Reg(next = replay_next)
  val s2_uncached_data_beat = RegEnable(tl_out(0).d.bits.data, replay_next)
  when (doUncachedResp) {
    assert(!s2_valid_hit)
  ...
  // gate the clock
  clock_en_reg := io.ptw(0).customCSRs.disableDCacheClockGate ||
 ...
 // io.cpu.perf.grant := d_done
  //io.cpu.perf.tlbMiss := io.ptw(0).req.fire()
    ...
    }
  {
    ...
    io.errors.bus.valid := tl_out(0).d.fire() && (tl_out(0).d.bits.denied || tl_out(0).d.bits.corrupt)
