...
package freechips.rocketchip.device

//import org.chipsalliance.cde.config.Parameters
import freechips.rocketchip.config.Parameters
import chisel3._
import chisel3.experimental.{ExtModule, prefix, HasExtModuleInline}
...
class AXI4MemoryImp[T <: Data](outer: AXI4Memory) extends AXI4SlaveModuleImp(outer) {
  ...
  //
  // Each ID needs its own FIFO of (address, len), not a single overwritable slot: AXI4UserYanker
  // upstream (amba/axi4/UserYanker.scala) allows up to `maxFlight` concurrent transactions per
  // ID, tracked via its own per-ID queue -- that per-ID flight cap is the entire reason
  // UserYanker exists (it lets many wider TileLink source IDs share a narrow AXI ID space). A
  // single slot per ID would return the wrong address's data for an earlier same-ID request once
  // a second one lands while the first is still outstanding.
  class ReadMeta extends Bundle {
    val addr = UInt((in.ar.bits.addr.getWidth - ramIndexBits).W)
    val len  = UInt(in.ar.bits.len.getWidth.W)
  }
  val readMetaQueues = Seq.fill(numOutstanding)(Module(new Queue(new ReadMeta, entries = 8)).io)
  ...
  readMetaQueues.zipWithIndex.foreach { case (q, id) =>
    ...
    q.enq.bits.len   := read_req_bits.len
  }
  ...
  // Never accept a new read for an ID whose per-ID queue has no room -- i.e. never let a
  // same-ID read overtake one that's still outstanding beyond what the queue can track.
  in.ar.ready := (!pending_read_req_valid || pending_read_req_ready) && read_meta_enq_ready

  // accept a write request (including address and data) and send it to the external model
  val pending_write_req_valid = RegInit(VecInit(Seq.fill(2)(false.B)))
  ...
  // Peek (not pop) the front of the responding ID's queue for every beat of the burst --
  // Queue.io.deq.bits holds steady at the front element until an actual deq fire advances it,
  // which we only do once, below, on the burst's last beat.
  val read_resp_meta = VecInit(readMetaQueues.map(_.deq.bits))(r_resp.bits.id)
  val read_resp_addr = read_resp_meta.addr + rdata_cnt.value
  val read_resp_len = read_resp_meta.len
  // Only actually pop the queue entry once the whole burst for it has been delivered --
  // read_resp_addr/read_resp_len above are read from the front entry on every beat.
  readMetaQueues.zipWithIndex.foreach { case (q, id) =>
    q.deq.ready := read_resp_last && r_resp.bits.id === id.U
  }
